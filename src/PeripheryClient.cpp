...
int PeripheryClient::GetCommandSocket() {
  ...
  std::cout << "Server address is " << inet_ntoa(server_addr.sin_addr) << ':'
            << htons(server_addr.sin_port) << std::endl;
...
std::string PeripheryClient::GetAvailableModels() {
  ...
  memcpy(&header[sizeof(UdpSignature)], ModelListSignature,
         sizeof(ModelListSignature));
  memcpy(request, header, headerSize);

  int bytes = SendReceive(sock, &fd, &server_address, request, headerSize,
...
bool PeripheryClient::SwitchModel(std::string modelName) {
  ...
  size_t headerSize = sizeof(header);
  memcpy(&header[0], UdpSignature, sizeof(UdpSignature));
  memcpy(&header[sizeof(UdpSignature)], SelectModelSignature,
         sizeof(SelectModelSignature));
  memcpy(request, header, headerSize);
  ...
  int bytes = SendReceive(sock, &fd, &server_address, request,
                          headerSize + payloadSize, response, sizeof(response));
  if (!bytes)
    clientConnected = false;
...
PeripherySession PeripheryClient::CreateInferenceSession() {
  ...
  size_t headerSize = sizeof(header);
  memcpy(&header[0], UdpSignature, sizeof(UdpSignature));
  memcpy(&header[sizeof(UdpSignature)], StartSessionSignature,
         sizeof(StartSessionSignature));
  memcpy(request, header, headerSize);
  ...
  if (!memcmp(header, response, headerSize)) {
    ...
    std::cout << "Session address is " << inet_ntoa(session_addr.sin_addr)
              << ':' << htons(session_addr.sin_port) << std::endl;
    uint32_t id;
...
bool PeripheryClient::SessionAvailable(uint32_t id) {
  ...
  size_t headerSize = sizeof(header);
  memcpy(&header[0], UdpSignature, sizeof(UdpSignature));
  memcpy(&header[sizeof(UdpSignature)], QuerySessionSignature,
         sizeof(QuerySessionSignature));
  memcpy(request, header, headerSize);

  memcpy(request + headerSize, &id, 4);

  int bytes = SendReceive(sock, &fd, &server_address, request, headerSize + 4,
                          response, sizeof(response));
  if (!bytes)
    clientConnected = false;
...
bool PeripheryClient::GetClientConnected() { return clientConnected; }
