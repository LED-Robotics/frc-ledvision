...
MLDetectionFrame generateMLFrame(det::BoxObject &det, uint8_t camId,
                                 uint32_t capTime) {
  return {
      (uint8_t)det.label, camId,          capTime,         det.rect.x,
      det.rect.y,         det.rect.width, det.rect.height,
  };
}

MLDetectionFrame generateMLFrame(det::PoseObject &det, uint8_t camId,
                                 uint32_t capTime) {
  return {
      (uint8_t)det.label, camId,          capTime,         det.rect.x,
      det.rect.y,         det.rect.width, det.rect.height,
...
int main(int argc, char **argv) {
  ...
  while (true) {
    ...
    if (recordState != lastRecordState) {
      ...
      for (Camera &cam : cameras) {
        if (recordState) {
          std::string newPath = orig.substr(0, orig.size() - 4) + "_id_" +
                                std::to_string(cam.GetID()) + ".avi";
    ...
    targetTags.clear();
    targetTags.insert(targetTags.end(), requestedTags.begin(),
                      requestedTags.end());
    for (Camera &cam : cameras) {
      cam.SetTargetTags(targetTags);
      auto id = cam.GetID();
      uint8_t found =
          count(camAprilTagDisabled.begin(), camAprilTagDisabled.end(), id);
      if (found)
    ...
    if (targetCount != currentSize) {
      free(tagBuffer);
      tagBufSize = sizeof(GlobalFrame) +
                   ((TAG_FRAME_SIZE + 2) * currentSize * cameras.size());
      /*std::cout << "Size of buffer changed: " << (int)tagBufSize <<
       * std::endl;*/
    ...
    if (camsInferencing != currentSize) {
      free(mlBuffer);
      mlBufSize = sizeof(GlobalFrame) +
                  ((ML_FRAME_SIZE + 2) * currentSize * maxDetections);
      /*std::cout << "Size of buffer changed: " << (int)tagBufSize <<
       * std::endl;*/
    ...
    for (Camera &cam : cameras) {
      ...
      uint8_t found =
          count(camAprilTagDisabled.begin(), camAprilTagDisabled.end(), camId);
      if (found)
        continue;
      if (!cam.GetTagDetectionCount())
        continue;
      auto tagDetections = cam.GetTagDetections();
      auto capTime = cam.GetCaptureTime();
      /*cam.PauseTagDetection();*/
      for (Camera::TagDetection &det : *tagDetections) {
        if (tagBufPos + TAG_FRAME_SIZE > tagBufSize)
          continue; // whoopsie, this would overflow, skip
    ...
    for (Camera &cam : cameras) {
      ...
      if (cam.GetMLDetectionMode() == Camera::MLMode::Detect) {
        ...
        for (det::BoxObject &det : *mlDetections) {
          if (mlBufPos + ML_FRAME_SIZE > mlBufSize)
            continue; // whoopsie, this would overflow, skip
